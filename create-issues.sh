...
# 使い方: gh auth login 後に ./create-issues.sh を実行

set -e

echo "=== Phase 1: TypeScript アップデート ==="
gh issue create \
  --title "Phase 1: TypeScript を 5.9 にアップデート" \
  --label "dependencies" \
  --body "$(cat <<'EOF'
## 概要
...
echo "=== Phase 2: Vite + プラグインアップデート ==="
gh issue create \
  --title "Phase 2: Vite 7 + プラグインをアップデート" \
  --label "dependencies" \
  --body "$(cat <<'EOF'
## 概要
...
echo "=== Phase 3: React アップデート ==="
gh issue create \
  --title "Phase 3: React 19 にアップデート" \
  --label "dependencies" \
  --body "$(cat <<'EOF'
## 概要
...
- `useRef` に引数が必須
- `act()` のインポート元変更
- その他: https://react.dev/blog/2024/04/25/react-19-upgrade-guide
...
echo "=== Phase 4: 型定義アップデート ==="
gh issue create \
  --title "Phase 4: 型定義パッケージをアップデート" \
  --label "dependencies" \
  --body "$(cat <<'EOF'
## 概要
...
echo "=== Phase 5: ビルド・動作確認 ==="
gh issue create \
  --title "Phase 5: 全体ビルド・動作確認" \
  --label "dependencies" \
  --body "$(cat <<'EOF'
## 概要
